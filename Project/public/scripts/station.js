(() => {
    ...
    async function request(url, options = {}) {
        ...
        if (!response.ok) {
            ...
            throw new Error(
                message || 'Station data could not be loaded.'
            );
        }
    ...
    function renderZones(zones) {
        ...
        if (!zones.length) {
            container.innerHTML =
                '<p class="empty-state">No zones are available.</p>';
            return;
        }
    ...
    function renderStations(stations) {
        const container =
            document.getElementById('stationsTable');

        if (!stations.length) {
            container.innerHTML =
                '<p class="empty-state">No stations match this selection.</p>';
            return;
        }
        ...
        <tr>
            ...
            <td>
                ${station.wheelchairAccessibility === 'Y'
                ? 'Yes'
                : 'No'}
            </td>
        </tr>
    ...
    function populateZoneFilter(zones) {
        const filter =
            document.getElementById('stationZoneFilter');
    ...
    document
        .getElementById('stationZoneFilter')
        .addEventListener(
            'change',
            async event => {
                try {
    ...
    document
        .getElementById('refreshStationDataButton')
        .addEventListener(
            'click',
            refreshAll
        );
